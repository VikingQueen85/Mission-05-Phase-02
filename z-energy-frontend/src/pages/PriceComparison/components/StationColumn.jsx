  ...
  const fetchStationDetails = async (slug, isInitialLoad = false) => {
    ...
    try {
      ...
      if (fallbackData) {
        ...
        // If API fails and no fallback, ensure stationDetails is reset
        setStationDetails(null);
  ...
  return (
    <div className={styles.stationColumn}>
      ...
      <div className={styles.cardsContainer}>
        {stationDetails && stationDetails.fuels && Array.isArray(stationDetails.fuels) &&
          ...
          {!isLoading && !error && stationDetails && (!stationDetails.fuels || stationDetails.fuels.length === 0) && (
              <p className={styles.noFuelsMessage}>No fuel prices available for this station.</p>
          )}
      </div>
    </div>
  );
